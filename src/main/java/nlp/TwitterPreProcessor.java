...
import nlp.tweetanalyzers.TweetSentimentAnalyzer;
...
public class TwitterPreProcessor {
    ...
    private final String USER_NAME_REGEX = "(^|[^@\\w])@(\\w{1,15})\\b";
...
//    public static final String WEB_URL_REGEX = "[-a-zA-Z0-9@:%._\\+~#=]{2,256}\\.[a-z]{2,6}\\b([-a-zA-Z0-9@:%_\\+.~#?&//=]*)";
    private final String WEB_URL_REGEX = "(http(s)?:\\/\\/)?(www\\.)?[-a-zA-Z0-9@:%._\\+~#=]{1,256}\\.[a-z]{2,6}\\b([-a-zA-Z0-9@:%_\\+.~#?&//=]*)";

    //source http://erictarn.com/post/1060722347/the-best-twitter-hashtag-regular-expression
    private final String HASH_TAG_REGEX = "\\B#\\w*[a-zA-Z]+\\w*";
    ...
    private final String REPEATED_PUNCTUATION_REGEX = "((?<!:)[^\\p{L}\\p{N}])\\1+";

    //source https://stackoverflow.com/questions/27951597/how-can-i-replace-all-duplicate-punctuation-with-single-punctuation-in-php
    // I looooooooove baseballllllll -> I love baseball;
    private final String REPEATED_CHARACTERS_REGEX = "";

    Pattern userNamePattern;
    Pattern webUrlPattern;
    Pattern hashTagPattern;
    Pattern repeatedPunctuationPattern;

    TwitterPreProcessor(){
        userNamePattern = Pattern.compile(USER_NAME_REGEX);
        webUrlPattern = Pattern.compile(WEB_URL_REGEX);
        hashTagPattern = Pattern.compile(HASH_TAG_REGEX);
        repeatedPunctuationPattern = Pattern.compile(REPEATED_PUNCTUATION_REGEX);
    }

    public SanitizedTweet sanitizeTweet(Tweet tweet){
    ...
    public static void main(String args[]){
        ...
        for (int i = 0; i < tweets.size(); i++) {
            ...
            sanitizedTweet = twitterPreProcessor.sanitizeTweet(tweet);

            //removed tweets with less then 10 characters of sanitized text. Little text might screw up nlp analyzers
            if (10 < sanitizedTweet.getSanitizedText().trim().length())
